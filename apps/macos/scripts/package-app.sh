...
if [[ -e "$APP" || -e "$DMG" ]]; then
  echo "版本 $APP_VERSION 已存在: $APP 或 ${DMG}。请设置新的 APP_VERSION，例如 APP_VERSION=0.13 ./scripts/package-app.sh" >&2
  exit 1
fi
...
- 0.14 优化录入页布局，支持 Markdown 快捷按钮、Enter 提交、Shift+Enter 换行、粘贴图片进附件、剪贴板小按钮、项目关联与回车提交保护。
